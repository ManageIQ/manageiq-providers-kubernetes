describe ContainerNode do
  it "has distinct routes" do
    ...
    :container_service,
    :name => "s",
    :container_routes => [FactoryBot.create(:container_route, :name => "rt")]
    )
    expect(FactoryBot.create(
    :container_node,
    :name => "n",
    :container_groups => [FactoryBot.create(:container_group, :name => "g1", :container_services => [service]),
                          FactoryBot.create(:container_group, :name => "g2", :container_services => [service])]
  ...
  describe "#external_logging_path" do
    ...
    it "will query for nothing when no name/fqdn is available" do
      node = FactoryBot.create(:container_node, :name => "")
    ...
    it "queries both fqdn and hostname when both are avaialble only from kubernetes lables" do
      node = FactoryBot.create(:container_node, :name => "other_name")
    ...
    it "queries both fqdn and hostname when both are avaialble" do
      node = FactoryBot.create(:container_node, :name => "hello.world.com")
      query = get_query(node.external_logging_path)
      expect(query).to eq("bool:(filter:(or:!((term:(hostname:'hello.world.com')),(term:(hostname:'hello')))))")
    end

    it "queries only for the name/fqdn when hostname can't be parsed" do
      node = FactoryBot.create(:container_node, :name => "hello")
      query = get_query(node.external_logging_path)
